...
export default function OrderTrackingPage() {
  ...
  // If there is no active order (e.g., user opened page directly), show an empty state
  if (!activeOrder || !activeOrder.items || activeOrder.items.length === 0) {
    return (
      <div className="bg-background-light dark:bg-background-dark text-[#181112] dark:text-white min-h-screen flex flex-col items-center justify-center text-center font-sans px-6">
        <div className="inline-flex items-center justify-center size-20 bg-yellow-100 text-yellow-600 rounded-full mb-6">
          <span className="material-symbols-outlined text-5xl">info</span>
        </div>
        <h1 className="text-3xl md:text-4xl font-extrabold mb-2">No Active Order</h1>
        <p className="text-gray-500 dark:text-gray-400 max-w-md mb-8">You don't have a current order to track. Add some delicious items to your cart and place an order to see live tracking and status here.</p>
        <Link href="/menu" className="bg-primary hover:bg-red-600 text-white font-bold px-8 py-3 rounded-xl shadow-lg shadow-primary/25 transition-all cursor-pointer">
          Browse Menu
        </Link>
      </div>
    );
  ...
  const { id, items, total, date } = activeOrder;
    ...
    <div className="bg-background-light dark:bg-background-dark text-[#181112] dark:text-white min-h-screen flex flex-col font-sans">
      <main className="flex-grow w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        ...
        <div className="mb-10 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-lg">Order ID: <span className="font-mono font-bold text-[#181112] dark:text-white">{id}</span></p>
          <p className="text-gray-400 text-sm mt-1">Placed at: {date}</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          {/* Left Column: Tracking & Timeline (static visual placeholders) */}
          ...
          {/* Right Column: Order Details - dynamic from activeOrder */}
          <div className="lg:col-span-5 space-y-6">
            <div className="sticky top-24 space-y-6">
                  ...
                  {items.map((it) => (
                    <div key={it.id} className="flex gap-4 group">
                      <div className="size-14 rounded-lg bg-gray-100 dark:bg-black/20 overflow-hidden flex-shrink-0">
                        <img alt={it.name} className="w-full h-full object-cover" src={it.image} />
                      </div>
                      <div className="flex-1">
                        <div className="flex justify-between items-start mb-1">
                          <h4 className="font-bold text-[#181112] dark:text-white text-sm">{it.name} <span className="text-gray-400 font-normal">x{it.quantity}</span></h4>
                          <span className="font-bold text-[#181112] dark:text-white text-sm">${(it.price * it.quantity).toFixed(2)}</span>
                        </div>
                        {/* Optionally list extras/notes if you later store them per item */}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Address Info (static placeholder) */}
              <section className="bg-white dark:bg-[#33181c] rounded-2xl shadow-sm border border-gray-100 dark:border-[#452026] overflow-hidden p-6">
